/*
 * Pointer-follow glow effect for the .banner CTA block.
 *
 * Exposes window.CCLK.initLiquidBanner(); assets/include.js calls it once.
 * No-ops on pages without a .banner (e.g. contact.html).
 */
(function () {
  const NS = (window.CCLK = window.CCLK || {});

  NS.initLiquidBanner = function initLiquidBanner() {
    if (NS._liquidBannerDone) return;

    const banner = document.querySelector('.banner');
    if (!banner) return;
    NS._liquidBannerDone = true;
