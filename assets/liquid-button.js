/*
 * Pointer-proximity "liquid" tilt/glow effect for .btn elements, plus a
 * one-shot vibrate on the primary [data-cta="request-call"] buttons.
 *
 * Exposes window.CCLK.initLiquidButtons(); assets/include.js calls it once,
 * after shared partials (which contain more .btn elements) are injected.
 */
(function () {
  const NS = (window.CCLK = window.CCLK || {});

  NS.initLiquidButtons = function initLiquidButtons() {
    if (NS._liquidButtonsDone) return;

    const buttons = Array.from(document.querySelectorAll('.btn'));
    if (!buttons.length) return; // nothing yet; allow a later retry
    NS._liquidButtonsDone = true;
