...
│   └── whatsapp-icon.png      # WhatsApp toggle/panel icon (generated asset, see scripts/)
├── scripts/                  # One-off/maintenance scripts — NOT loaded by any page
│   └── make-whatsapp-icon.py  # Regenerates assets/whatsapp-icon.png (needs Pillow)
└── CLAUDE.md                # This file
```
...
(`assets/whatsapp-icon.png`) instead of an inline `<svg>` — a plain white
"line icon" style glyph (an outlined chat bubble with a solid phone-handset
silhouette inside), not the official multi-color WhatsApp logo. It's a
**generated** asset: `scripts/make-whatsapp-icon.py` (Pillow, run manually,
not part of the site's runtime) draws it procedurally and is the source of
truth for changing it — don't hand-edit the PNG; edit the script and rerun
it. The image sits inside `.toggle-icon`/`.whatsapp-panel__avatar`, which
already provide their own translucent-white circular backing and the
button's green gradient background, so the plain white PNG needs no
background of its own — `.toggle-icon img`/`.whatsapp-panel__avatar img` in
`styles.css` just size it (22px / 26px respectively).
...
| WhatsApp icon (white line-icon PNG) | `assets/whatsapp-icon.png` (generated by `scripts/make-whatsapp-icon.py`) |
...
- Polished the About section: bigger feature icons with a hover scale, a
  ...
  section" above).
- Replaced the WhatsApp toggle/panel icon — previously the official
  multi-color logo as inline `<svg>` — with a generated white line-icon PNG
  (`assets/whatsapp-icon.png`, source script `scripts/make-whatsapp-icon.py`)
  (see "WhatsApp icon" above).
